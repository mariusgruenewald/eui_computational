...
function EGM(prim::Primitive, wage::Float64, r::Float64)
    ...
    @unpack nz, na, β, σ, z_grid, trans_mat, a_grid_log, tol  = prim
...
function calc_EEE(prim::Primitive, na_precise::Int64, kpol::Matrix{Float64}, r::Float64, wage::Float64)
    ...
    @unpack minval, maxval, nz, na, β, σ, trans_mat, z_grid, a_grid_log = prim
    # make fine grid
    a_grid = copy(a_grid_log)
...
function young_2010_continuous(prim::Primitive, kpol::Matrix{Float64})
    ...
    @unpack nz, na, trans_mat, a_grid_log, tol = prim
...
function young_2010_discrete(prim::Primitive, policy_a::Matrix{Float64})
    ...
    @unpack nz, na, trans_mat, maxiter = prim
...
function solve_partial_model(VFI_bool::Bool)
    ...
    if VFI_bool == true
        ...
        # Consumption Levels
        cons_levels = prim.z_grid*wage .+ (1+r)*policy_a .- prim.a_grid_log'
...
#### Value Function Iteration, for log grids it's just too slow. Works for lingrids though
@time dist_vfi, policy_a, agg_k_hh_vfi, r_vfi, wage_vfi  = solve_general_model(true, 0.3)

dist_vfi_plot = plot(prim.a_grid_log, dist_vfi', label = [L"z_1" L"z_2"], dpi=300, title = "Probability of Assets by Productivity VFI")
xaxis!("Assets")
yaxis!("Probability")
savefig(dist_vfi_plot,"dist_vfi.png")

ergodic_dist_vfi = plot(prim.a_grid_log, dist_vfi'.*policy_a', label = [L"z_1" L"z_2"], dpi=300, title = "Distribution of Assets by Productivity VFI")
xaxis!("Assets")
yaxis!("Distribution")
...
dist_egm_plot = plot(prim.a_grid_log, dist_egm', label = [L"z_1" L"z_2"], dpi=300, title = "Probability of Assets by Productivity EGM")
xaxis!("Assets")
yaxis!("Probability")
savefig(dist_egm_plot,"dist_egm.png")

ergodic_dist_egm = plot(prim.a_grid_log, dist_egm'.*kpol', label = [L"z_1" L"z_2"], dpi=300, title = "Distribution of Assets by Productivity VFI")
xaxis!("Assets")
yaxis!("Distribution")
